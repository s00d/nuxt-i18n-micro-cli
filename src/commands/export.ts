...
export default defineCommand({
  meta: {
    name: 'export',
    description: 'Export translations and organize as PO files',
  },
  args: {
    ...sharedArgs,
    prod: {
  ...
  async run({ args }: { args: { cwd?: string, logLevel?: string, potsDir: string } }) {
    ...
    // Сохранение в формат PO
    Object.entries(localeObjects).forEach(([locale, translation]) => {
      const { global, pageSpecific } = translation as LocaleTranslation;

      const globalPOBuffer = convertToPO(global)
      const globalPOPath = path.join(potsDir, `${locale}.po`)
      fs.writeFileSync(globalPOPath, globalPOBuffer)

      Object.entries(pageSpecific).forEach(([page, translations]) => {
        const pagePOBuffer = convertToPO(translations)
        const pagePOPath = path.join(potsDir, 'pages', page, `${locale}.po`)
        ensureDirectoryExists(path.dirname(pagePOPath))
        fs.writeFileSync(pagePOPath, pagePOBuffer)
      })
    })

    consola.log('Locale-specific translations have been saved to PO files.')
  },
})
